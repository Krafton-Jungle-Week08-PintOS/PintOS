...
{										  // thread 초기화
	ASSERT(intr_get_level() == INTR_OFF); // 인터럽트가 비활성화된 상태인지 확인(조건이 참인지만 확인하는 매크로)

	/* Reload the temporal gdt for the kernel
	 * This gdt does not include the user context.
	 * The kernel will rebuild the gdt with user context, in gdt_init (). */
	// 메모리 세그먼트의 정보를 저장하는 구조체
	struct desc_ptr gdt_ds = {						   // 디스크립터(글로벌 디스크립터 테이블)의 구조체를 초기화 하는 작업
							  .size = sizeof(gdt) - 1, // (range 개념과 비슷하다 0~8바이트 이기 때문에 크기 필드는 -1 하면 7로 설정됨) 유효 바이트 0~7
	...
	initial_thread = running_thread();				  // 실행 중인 쓰레드를 initial_thread에 저장
	init_thread(initial_thread, "main", PRI_DEFAULT); // 쓰레드 구조체 초기화, "이름", 기본 우선순위 설정
	initial_thread->status = THREAD_RUNNING;
	initial_thread->tid = allocate_tid(); // 고유한 쓰레드ID 할당
}
...
void thread_start(void)
{
	...
	struct semaphore idle_started;						 // 쓰레드 간의 동기화를 위한 세마포어 구조체
	sema_init(&idle_started, 0);						 // 세마포어를 0으로 초기화
	thread_create("idle", PRI_MIN, idle, &idle_started); // 우선 순위가 낮은 쓰레드 생성

	/* Start preemptive thread scheduling. */
	intr_enable(); // 인터럽트 활성화

	/* Wait for the idle thread to initialize idle_thread. */
	sema_down(&idle_started); // 세마포어가 0이 아니면 실행, 0이면 대기
}
...
{ // 인자를 받지 않으면 재우는 시간을 지정할 수 없음
	...
	curr->awake_ticks = ticks;	// 깨워야 할 시점을 awake_ticks에 저장ehldjdi
