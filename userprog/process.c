 ...
 * Returns -1 on fail. */
// user program에 대한 인수를 설정한다.(execute)
int process_exec(void *f_name)
{
	...
	// 사용자 프로그램이 실행될 때 필요한 CPU 상태를 정의한다.
...
int parse_arg(char *file_name, char **arg_value)
{

	int arg_count = 0; // argument 개수
	char *token;
	char *save_ptr; // 분리된 문자열 중 남는 부분의 시작주소
...
void stack_arg(char **arg_value, int arg_count, struct intr_frame *if_)
{
	if_->rsp = USER_STACK;
	// arg_value = arg를 파싱해서 배열에 담음(더블포인터로 받아서 역참조하자
	// count = arg수, if = 레지스터에 담을 인자를 구조체로 선언)
	char *arg_addr[64];
...
// intr_frame 구조체는 인터럽트가 발생했을 때 CPU 상태를 저장하는 역할
// 인터럽트가 처리된 후 저장된 레지스터 값들을 복원해서 중단된 작업을 다시 실행할 수 있게 한다.
{
	...
	bool success = false;
	int i;
	char *arg_value[128]; // parsing할 argument 배열
	int arg_count;
